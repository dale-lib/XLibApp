...
import com.dale.dialog.photoview.OnMatrixChangedListener;
import com.dale.dialog.photoview.PhotoView;
import com.dale.glide.GlideApp;
import com.dale.glide.GlideOptions;
...
public class SmartGlideImageLoader implements XPopupImageLoader {
    ...
    public SmartGlideImageLoader(@DrawableRes int errImgRes) {
        errImg = errImgRes;
    }
    ...
    public View loadImage(final int position, @NonNull final Object url, @NonNull ImageViewerPopupView popupView,
        ...
        GlideApp.with(imageView).downloadOnly().load(url)
                        ...
                        if (imageView instanceof PhotoView) {
                            ...
                            if (size[0] > maxW || size[1] > maxH) {
                                ...
                                GlideApp.with(imageView).load(resource)
                                        ...
                                        .apply(GlideOptions.errorOf(errImg).override(size[0], size[1])).into(((PhotoView) imageView));
                            }
                        } else {
                            //大图加载
...
//                    + "  dy: "+ (newCenter.y - ssiv.getMeasuredHeight()/2));
    ...
    public void loadSnapshot(@NonNull Object uri, @NonNull final PhotoView snapshot, @Nullable final ImageView srcView) {
        if (mBigImage) {
            ...
            GlideApp.with(snapshot).downloadOnly().load(uri)
                    .into(new ImageDownloadTarget() {
                        @Override
                        public void onLoadFailed(Drawable errorDrawable) {
                            super.onLoadFailed(errorDrawable);
                        ...
                        public void onResourceReady(@NonNull File resource, Transition<? super File> transition) {
                            ...
                            if (size[0] > maxW || size[1] > maxH) {
                                ...
                                GlideApp.with(snapshot).load(resource).apply(new GlideOptions().override(size[0], size[1])).into(snapshot);
                            }
                        }
                    });
        } else {
            GlideApp.with(snapshot).load(uri).override(Target.SIZE_ORIGINAL).into(snapshot);
        }
    }
